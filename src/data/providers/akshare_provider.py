...
class AKShareProvider(BaseDataProvider):
    ...
    _industry_stock_snapshot_path = (
        Path(__file__).resolve().parents[3] / "cache" / "industry_stock_cache.json"
    )
    _industry_meta_heatmap_fallback_path = (
        Path(__file__).resolve().parents[3] / "data" / "industry" / "heatmap_history.json"
    )
    ...
    def __init__(self, api_key: Optional[str] = None, config: Dict[str, Any] = None):
        ...
        self._akshare_breakers: Dict[str, CircuitBreaker] = {}
    ...
    def _call_akshare(self, breaker_key: str, fn, *args, **kwargs):
        breaker = self._akshare_breakers.get(breaker_key)
        if breaker is None:
            breaker = CircuitBreaker(
                failure_threshold=5,
                recovery_timeout=60.0,
                name=f"akshare.{breaker_key}",
            )
            self._akshare_breakers[breaker_key] = breaker
        return breaker.call(fn, *args, **kwargs)

    def get_circuit_status(self) -> Dict[str, Any]:
        return {name: breaker.status() for name, breaker in self._akshare_breakers.items()}
    ...
    def _get_industry_stock_cache_key(
        ...
        return (
            f"{str(industry_name or '').strip()}|market_cap:{int(bool(include_market_cap_lookup))}"
        )
    ...
    @classmethod
    def _load_persistent_industry_stock_snapshot(
        cls,
    ...
    def _get_persistent_industry_stock_snapshot(
        ...
        if datetime.now() - updated_at >= timedelta(
            hours=cls._industry_stock_snapshot_stale_after_hours
        ):
            return None
    ...
    def _persist_industry_stock_snapshot(
        ...
        with cls._industry_stock_snapshot_lock:
            ...
            if isinstance(existing_entry, dict) and list(
                existing_entry.get("stocks") or []
            ) == list(stocks):
                return
            snapshot[cache_key] = {
        ...
        try:
            ...
            try:
                ...
                if not df_meta.empty and "total_market_cap" in df_meta.columns:
                    ...
                    df = df.merge(
                        df_meta[
                            ["industry_name", "original_name", "total_market_cap", "turnover_rate"]
                        ],
                    ...
                    if "industry_name_y" in df.columns:
                        ...
                        df = df.drop(
                            columns=["industry_name_x", "industry_name_y"], errors="ignore"
                        )
                    ...
                    df["total_market_cap"] = pd.to_numeric(
                        df["total_market_cap"], errors="coerce"
                    ).fillna(0)
            ...
            # 使用主力净流入绝对值作为相对大小代理
            if not meta_merged or (
                "total_market_cap" in df.columns and (df["total_market_cap"] == 0).all()
            ):
    ...
    def _get_industry_metadata(self) -> pd.DataFrame:
        ...
        if not hasattr(self, "_industry_meta_cache"):
            ...
            if self._industry_meta_cache is None:
                ...
                if persistent_df is not None and not persistent_df.empty:
                    ...
                    self._industry_meta_cache_time = persistent_time or datetime.now() - timedelta(
                        days=1
                    )
        ...
        if (
            self._industry_meta_cache is not None
            and self._industry_meta_cache_time
            and current_time - self._industry_meta_cache_time < timedelta(hours=4)
        ):
        ...
        if (
            ...
            if self._industry_meta_cache is not None:
                logger.info(
                    "Skipping AKShare metadata refresh during cooldown; using in-memory snapshot"
                )
                return self._industry_meta_cache
            persistent_df, persistent_time = self._load_persistent_industry_metadata()
            if persistent_df is not None and not persistent_df.empty:
                ...
                cls._shared_industry_meta_cache_time = self._industry_meta_cache_time
                logger.info(
                    "Skipping AKShare metadata refresh during cooldown; using persistent snapshot"
                )
            ...
            if heatmap_df is not None and not heatmap_df.empty:
                ...
                cls._shared_industry_meta_cache_time = self._industry_meta_cache_time
                logger.info(
                    "Skipping AKShare metadata refresh during cooldown; using heatmap-history snapshot"
                )
                return heatmap_df
            logger.info(
                "Skipping AKShare metadata refresh during cooldown; no fallback snapshot available"
            )
            return pd.DataFrame()
        ...
        with cls._industry_meta_lock:
            ...
            for attempt in range(2):
                try:
                    ...
                    logger.info(
                        f"Fetching industry metadata from AKShare (attempt {attempt + 1})..."
                    )
                    df_meta = self._call_akshare(
                    ...
                    logger.info(
                        f"Industry metadata fetched successfully: {len(df_meta)} industries"
                    )
        ...
        if persistent_df is not None and not persistent_df.empty:
            ...
            cls._shared_industry_meta_cache_time = self._industry_meta_cache_time
            logger.warning(
                f"Using persistent metadata snapshot as fallback (last error: {last_error})"
            )
            return persistent_df
        heatmap_df, heatmap_time = self._load_heatmap_history_metadata_fallback()
        if heatmap_df is not None and not heatmap_df.empty:
            ...
            cls._shared_industry_meta_cache_time = self._industry_meta_cache_time
            logger.warning(
                f"Using heatmap-history metadata snapshot as fallback (last error: {last_error})"
            )
            return heatmap_df
    ...
    def _get_all_stocks_spot(self) -> pd.DataFrame:
        ...
        if (
            not self._spot_cache.empty
            and self._spot_cache_time
            and current_time - self._spot_cache_time < timedelta(minutes=5)
        ):
        ...
        def _load_stocks() -> List[Dict[str, Any]]:
            ...
            try:
                ...
                try:
                    ...
                    if (
                        not df_meta.empty
                        and "industry_name" in df_meta.columns
                        and "original_name" in df_meta.columns
                    ):
                        ...
                        if not match.empty:
                            ...
                            if target_name != industry_name:
                                logger.info(
                                    f"Resolved industry name: {industry_name} -> {target_name}"
                                )
                except Exception as e:
                ...
                market_cap_map = (
                    self._get_all_stocks_market_cap() if include_market_cap_lookup else {}
                )
                ...
                for _, row in df.iterrows():
                    ...
                    if not market_cap:
                        market_cap = self._safe_float(row.get("总市值")) or self._safe_float(
                            row.get("流通市值")
                        )
                ...
                if soft_fail:
                    ...
                    logger.error(
                        f"Error fetching stocks for industry {industry_name} with no fallback: {e}"
                    )
                return []
    ...
    def get_stock_valuation(self, symbol: str, cached_only: bool = False) -> Dict[str, Any]:
        ...
        try:
            if cached_only:
                ...
                if not self._spot_cache_time or datetime.now() - self._spot_cache_time >= timedelta(
                    minutes=5
                ):
