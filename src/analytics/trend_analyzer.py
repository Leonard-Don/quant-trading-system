...
class TrendAnalyzer:
    ...
    def _identify_support_resistance(
        ...
        for i in range(window, len(df) - window):
            ...
            is_resistance = (
                highs.iloc[i] == highs.iloc[i - window : i + window + 1].max()
            )
    ...
    def _calculate_technical_score(
        ...
        indicators = {
            ...
            "macd_signal": round(
                float(signal.iloc[-1]) if not pd.isna(signal.iloc[-1]) else 0, 4
            ),
            ...
            "plus_di": round(
                float(current_pos_di) if not pd.isna(current_pos_di) else 0, 2
            ),
            "minus_di": round(
                float(current_neg_di) if not pd.isna(current_neg_di) else 0, 2
            ),
            "stoch_k": round(
                float(current_stoch_k) if not pd.isna(current_stoch_k) else 50, 2
            ),
            "stoch_d": round(
                float(current_stoch_d) if not pd.isna(current_stoch_d) else 50, 2
            ),
            "williams_r": round(
                float(current_williams) if not pd.isna(current_williams) else -50, 2
            ),
    ...
    def _multi_timeframe_analysis(self, close: pd.Series) -> Dict[str, str]:
        ...
        if data_len > short_period:
            short_trend = (
                "上涨" if close.iloc[-1] > close.iloc[-short_period - 1] else "下跌"
            )
            short_change = (
                (close.iloc[-1] - close.iloc[-short_period - 1])
                / close.iloc[-short_period - 1]
                * 100
            )
            timeframes["short"] = {
                "period": short_label,
        ...
        if data_len > mid_period:
            mid_trend = (
                "上涨" if close.iloc[-1] > close.iloc[-mid_period - 1] else "下跌"
            )
            mid_change = (
                (close.iloc[-1] - close.iloc[-mid_period - 1])
                / close.iloc[-mid_period - 1]
                * 100
            )
            timeframes["medium"] = {
                "period": mid_label,
        ...
        if data_len > long_period:
            long_trend = (
                "上涨" if close.iloc[-1] > close.iloc[-long_period - 1] else "下跌"
            )
            long_change = (
                (close.iloc[-1] - close.iloc[-long_period - 1])
                / close.iloc[-long_period - 1]
                * 100
            )
            timeframes["long"] = {
                "period": long_label,
    ...
    def _analyze_momentum(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "williams_r": round(
                float(current_williams) if not pd.isna(current_williams) else 0, 2
            ),
        }
    ...
    def _analyze_volatility(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "historical_volatility": round(
                float(current_volatility) if not pd.isna(current_volatility) else 0, 2
            ),
            "atr": round(float(current_atr) if not pd.isna(current_atr) else 0, 2),
            "atr_percent": round(
                float(atr_percent) if not pd.isna(atr_percent) else 0, 2
            ),
            "bollinger_width": round(
                float(bb_width) if not pd.isna(bb_width) else 0, 2
            ),
        }
