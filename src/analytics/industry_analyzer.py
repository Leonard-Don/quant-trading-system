...
class IndustryAnalyzer:
    ...
    def _merge_momentum_and_flow(self, momentum_df: pd.DataFrame, money_flow_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        return merge_momentum_and_flow(momentum_df, money_flow_df)
    ...
    def _normalize_money_flow_dataframe(
        ...
        - flow_strength
        """
        return normalize_money_flow_dataframe(money_flow_df, days)
    ...
    def calculate_industry_momentum(
        ...
        try:
            ...
            if not money_flow_df.empty and "change_pct" in money_flow_df.columns:
                ...
                # 小样本或定向计算时优先使用真实行业指数历史波动率；全市场首屏大样本仍保留代理值，
                # 避免冷启动时触发过多行业指数历史请求。
                should_fetch_historical_volatility = industries is not None or len(df) <= 20
