...
const MarketAnalysis = ({ symbol: propSymbol, embedMode = false }) => {
    ...
    const volumeContent = useMemo(() => {
        ...
        const {
            ...
            money_flow = { mfi: 0, status: 'neutral' },
            volume_patterns = { patterns: [] },
            obv_analysis = {}
        } = volume_analysis || {};

        const volumeTrend = normalizeVolumeTrend(rawVolumeTrend);
        const mfiValue = Number(money_flow.mfi ?? 0);
        const flowStatus = money_flow.status || 'neutral';
        const mfiColor = mfiValue > 80 ? '#ff3030' : mfiValue < 20 ? '#00b578' : '#1890ff';
                ...
                <Col xs={24} md={8}>
                    <Card title="量能趋势">
                        ...
                        <div style={{ marginTop: 10 }}>
                            ...
                            <Text strong>{volumeTrend.volume_ratio}x</Text>
                        </div>
                    </Card>
                </Col>
