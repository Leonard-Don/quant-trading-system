...
[本地体验](#-本地体验) · [策略证伪](#-策略证伪诚实结论) · [核心能力](#-核心能力) · [界面预览](#-界面预览) · [快速开始](#-快速开始) · [测试验证](#-测试验证) · [API 文档](#-api-文档) · [更新日志](docs/CHANGELOG.md)
...
> **先说清楚这个仓不是什么。** 仓库名里带 `trading`，但它**并不是一个能赚钱的交易系统**，也不声称能产生 alpha。它是一套量化**研究基础设施**：可独立运行的回测引擎、实时行情管线、29 种策略实现，以及——同样重要的——一套用来**证伪**自己策略的形式化统计方法学。仓内的统计检验已经证明：本仓的轮动策略相对买入持有**没有统计显著的超额收益**（详见下方 [📉 策略证伪：诚实结论](#-策略证伪诚实结论)）。请把它当成一个**研究与回测平台**来用，而不是一个交易信号源。
...
- 想看一个策略**被严格统计检验、并被诚实记录为"无显著边际"**的完整证据链——而不是又一个只贴漂亮收益曲线的仓
...
| 策略到底有没有超额收益 | [📉 策略证伪：诚实结论](#-策略证伪诚实结论) |
| 怎么最快启动 | [快速开始](#-快速开始) |
...
> 本仓的一个核心交付物，是**对自己策略的形式化证伪**。结论很直接：**在本数据样本上，ETF 轮动策略相对买入持有的优势，在统计上与噪声无法区分。**

很多量化项目只展示一条跑赢基准的收益曲线。本仓刻意走相反方向——用学术界标准的假设检验去**质疑**回测里那条看起来漂亮的曲线，并把不利结论原样写进仓库。

### 怎么测的

[`src/backtest/strategy_statistical_tests.py`](src/backtest/strategy_statistical_tests.py) 实现了三个**互补且正交**的形式化检验，全部作用于对齐后的日收益序列：

| 检验 | 出处 | 回答的问题 |
|------|------|-----------|
| **Diebold-Mariano** | Diebold & Mariano (1995)，配 Newey-West HAC 方差 | 两条策略收益序列的期望损失差是否为 0（对交易 P&L 的自相关做了修正） |
| **Politis-Romano 区块自举** | Politis & Romano (1994) | 对收益差做循环区块自举，保留短周期自相关，给出非参数置信区间与 p 值 |
| **Sharpe 比率差检验** | Memmel (2003) / Jobson-Korkie (1981) | 两个 Sharpe 是否相等的闭式统计量 |

多重检验用 Holm / Bonferroni 校正——任何"显著"结果都必须先扛过校正才算数。

### 结论（across 27 个 Walk-Forward 窗口）

完整结果见 [`docs/walkforward_stat_tests_summary.md`](docs/walkforward_stat_tests_summary.md)（明细 CSV：[`docs/walkforward_stat_tests.csv`](docs/walkforward_stat_tests.csv)）。把 2020-01 → 2026-05 的样本切成 27 个 2 年滚动窗口（步长 6 个月），对 `rotation` / `mean_reversion` / `blend` 三套策略各自对比买入持有：

- **27 个窗口里，原始 DM p < 0.05 的：0 个。** 经 Holm 家族校正后通过的：**0 个**。
- 最小 p 值仅 **0.1299**（远高于 0.05）。
- 方向一致性是掷硬币级别：`rotation` 仅 4/9 个窗口跑赢买入持有（44%），`mean_reversion` 3/9（33%）。

**一句话结论：** 跨全部 27 个 Walk-Forward 窗口，轮动策略相对买入持有的边际，在统计上与噪声无法区分——回测里 16–19pp 的原始价差，无论怎么切窗口都不显著。功效分析显示：若真存在 +3pp/年 的边际，需要约 6 年周频数据才能在 80% 功效下检出；当前 5 年样本是**功效不足**的。

> 复现：`python scripts/walkforward_stat_tests.py --csv data/etf_backtest/etf_prices_5y.csv --window-years 2 --step-months 6` ；终端期单窗检验见 `scripts/strategy_significance_test.py`。

这个结论**没有**让仓库变弱——一套能诚实证伪自身假设的研究框架，本身就是这个项目最有价值的部分。
...
- 配套 [`strategy_statistical_tests.py`](src/backtest/strategy_statistical_tests.py) 形式化检验层（DM + 区块自举 + Sharpe 检验 + Holm 校正），用于判断回测价差是否只是噪声——详见 [📉 策略证伪：诚实结论](#-策略证伪诚实结论)

<details>
<summary><b>📋 内置策略一览（29 种）</b></summary>
