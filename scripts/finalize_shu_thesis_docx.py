...
def append_thesis_expansion_content(doc: Document) -> None:
    expansions = [
        ...
        (
            ...
            "1.4 论文结构安排",
            [
                "具体来说，本文的研究内容可以分成四个层次。第一层是数据层，重点解决多源行业数据、成分股数据和财务行情字段如何进入统一结构；第二层是模型层，重点解释行业热度得分和龙头股得分怎样计算；第三层是服务层，说明接口缓存、回退和响应封装如何保证页面可用；第四层是展示层，通过热力图、排行榜和详情弹窗将结果呈现给使用者。",
...
def polish_final_submission_copy(doc: Document) -> None:
    ...
    exact_replacements = {
        ...
        "行业热点识别可以从三个角度理解。第一是资产定价角度，行业收益可能包含共同风险暴露和风格因子影响；第二是动量角度，强势行业可能在一段时间内继续受到资金关注；第三是行为金融角度，热点也可能包含情绪放大和过度反应。[8-20] 这三个角度共同说明，热门行业不能只靠单一指标判断。":
            "本文把行业热点理解为三个信号的交叉结果：首先，行业收益中可能包含共同风险暴露和风格因子影响；其次，行业动量研究说明强势板块在短窗口内可能延续资金关注；最后，行为金融研究提示短期热点也可能混入情绪放大和过度反应。[8-20] 因此，系统没有把热门行业简化为涨幅榜，而是要求价格、资金、活跃度和风险约束共同参与判断。",
        "本系统的关键技术可以概括为 Python 数据处理、机器学习辅助分析、Web 接口服务和前端可视化四类。Python 生态中的 pandas、NumPy、SciPy、Matplotlib 和 scikit-learn 已经形成较成熟的数据分析工具链，适合处理结构化行情、横截面标准化、聚类辅助分析和图表生成。[6-7][34-37]":
            "围绕行业热度识别这一主线，系统采用的技术可以分为四组。第一组是 pandas、NumPy 和 SciPy，用于把行业摘要、资金流、成分股和财务字段整理成可比较的横截面数据；第二组是 scikit-learn 与 Matplotlib，用于辅助聚类解释和图表生成；第三组是 FastAPI，用于把分析结果封装为前端可调用的接口；第四组是 React 与 Ant Design，用于承载热力图、排行榜和详情弹窗。[6-7][34-37]",
        "完成分层后，各层职责更加清晰。表现层主要负责图表交互和结果展示，服务层负责将分析结果组织为统一响应，分析层专注于评分计算与结果解释，数据层则通过名称映射、节点回退、符号缓存和过期缓存保障等机制提高数据可用性。后续若需要替换局部数据源或微调评分权重，改动也大多集中在分析层和数据层，无需整体重写前端页面。":
            "四层结构的作用在于把容易变化的部分拆开。表现层保留热力图、排行榜和详情弹窗的交互状态；服务层把行业名称、统计窗口和榜单类型转换为稳定接口响应；分析层负责把 IndustryAnalyzer 与 LeaderStockScorer 的计算结果解释出来；数据层则处理节点映射、主源失败、补充源回退和过期缓存。这样一来，后续若只调整 THS-first 取数字段或龙头股权重，通常不需要改动整套前端页面。",
        "当然，文件存储并不意味着可以忽略数据规模。热力图历史记录如果无限增长，会影响读取速度和版本管理；财务缓存如果长期不清理，也可能出现过期字段影响评分的问题。因此，项目对历史快照数量和缓存有效期都做了控制，并在论文中把它们作为工程取舍说明。":
            "采用文件存储并不代表可以放任数据无限累积。热力图历史如果持续追加，会拖慢读取和论文样例定位；财务缓存若长期不刷新，ROE、估值和成长字段也可能滞后于实际披露。为避免这些问题，系统在热力图历史记录数、文件大小和缓存有效期上设置了上限，并把这些限制写入论文，作为本地研究原型阶段的工程边界。",
        "分数压缩是页面展示中的一个重要细节。若直接展示标准化后的原始得分，使用者可能看到大量负值或极端值，不利于热力图颜色映射；压缩到相对稳定区间后，虽然牺牲了一部分数学精度，但更适合毕业设计中的可视化展示和人工解释。":
            "分数压缩主要服务于页面解释。横截面标准化后的原始值可能出现负数，也可能被单个极端行业拉开距离；若直接用于热力图，颜色差异会显得不稳定。系统因此把得分映射到约 20 至 95 的展示区间，让排行榜和热力图更容易阅读。需要注意的是，这一处理只改变展示尺度，不改变行业之间的相对排序逻辑。",
        "缓存策略同样嵌入在行业分析模块周围。对于短期重复请求，缓存可以减少外部接口压力；对于数据源短时失败，过期缓存能维持页面可用。系统不仅将缓存视为性能优化手段，也将其作为行业研究原型稳定运行的一部分。":
            "缓存不是附加在系统外侧的简单加速层，而是行业分析链路中的兜底机制。短时间重复打开热力图时，端点缓存可以避免反复请求外部接口；主源短时不可用时，过期缓存能先维持页面可读；成分股完整版尚未构建完成时，快速路径则先返回候选结果。对于毕业设计而言，这些缓存行为直接决定截图、快照和演示流程是否稳定。",
        "在完整评分链路中，系统会尽量补充估值和财务字段，再按六维权重生成综合得分。这个链路更适合详情页，因为使用者在详情页中会关注某只股票排序靠前的原因，以及它在哪些维度上占优。":
            "完整评分链路会先补齐估值、盈利、成长和行情字段，再按六维权重计算综合得分。该链路主要服务详情页：当某只股票进入 core 榜单后，页面需要说明其优势来自规模、估值、ROE、成长性还是短期动量，而不是只给出一个排序数字。",
        "接口缓存的设置也与页面使用方式有关。热门行业和热力图结果在短时间内可以复用，因此适合设置分钟级缓存；完整成分股和财务字段成本更高，适合异步构建或长期缓存；偏好配置则更强调写入后的稳定读取。不同接口采用不同缓存策略，比统一缓存时间更合理。":
            "接口缓存的粒度按照页面使用顺序来划分。热门行业和热力图属于首屏数据，适合短时间复用；完整成分股和财务字段拉取成本更高，适合异步构建或长期缓存；偏好配置则要求保存后能够稳定读取。系统没有采用统一缓存时间，而是按接口成本和交互频率分别设置策略。",
        "持续跟踪功能的意义在于把一次性结果变成可回看的研究材料。行业热度今天排在前列，并不意味着明天仍然如此；只有保留历史快照，使用者才能观察某一行业是短暂冲高、持续走强，还是在资金流和活跃度上出现分化。":
            "持续跟踪功能把一次性的行业排序保存为可回看的研究材料。某个行业在当前窗口排在前列，并不说明后续仍会维持强势；只有保存历史快照，才能比较其后续是短暂冲高、连续走强，还是在资金流和活跃度上出现背离。",
        "从论文角度看，持续跟踪功能也提高了样例分析的可信度。第六章固定引用的快照和龙头股样本，正是通过这种持久化能力保存下来的。如果没有这类记录，论文结果会随着数据源更新不断变化，不利于提交和答辩。":
            "从论文材料角度看，持续跟踪功能还保证了样例可复核。第六章使用的热力图快照和龙头股样本均来自保存下来的固定文件；如果没有这些记录，表格数值会随着外部数据源更新而漂移，评阅时也难以回到同一时点检查。",
        "功能测试还需要关注降级场景。金融数据接口在实际运行中可能出现空字段、超时或字段名变化，如果测试只覆盖理想返回，就难以证明系统具备真实可用性。因此，行业评分与接口测试都应检查字段缺失、缓存命中和代理值生效等情况。":
            "功能测试不能只覆盖理想返回。实际运行时，金融接口可能返回空字段、超时、字段名变化或节假日缺口；如果测试只验证一次正常响应，就难以说明系统具备连续使用能力。因此，本文把字段缺失、缓存命中、代理波动率生效和快速评分回退都纳入验证范围。",
        "样例中能源金属、元件、军工电子和小金属等方向位于前列，说明系统能够在固定窗口内捕捉阶段性强势板块。但这里仍然需要强调，该结论只对应样本时点。市场环境、政策预期、行业景气度和资金风格变化之后，排序结果都可能发生明显变化。":
            "从固定快照看，能源金属、元件、军工电子和小金属等方向在样本窗口内排序靠前，说明系统能够捕捉阶段性强势板块。不过，这一结论只对应保存快照的时点；如果市场环境、政策预期、行业景气度或资金风格发生变化，排序结果也会随之调整。",
        "本文的不足也比较明确。首先，行业热度和龙头股评分权重主要依据工程经验设定，尚未经过系统参数优化；其次，样例分析使用的是固定快照，不能替代长周期回测；再次，政策文本、新闻舆情和产业链上下游关系尚未进入核心得分。明确上述边界，有助于提高论文结论的稳健性，也为后续迭代提供了方向。":
            "本文仍有若干边界需要说明。其一，行业热度和龙头股权重来自文献口径、工程经验和样例调试，尚未通过滚动窗口系统寻优；其二，第六章使用固定快照说明系统输出，不能替代覆盖牛熊阶段的长期回测；其三，政策文本、新闻舆情和产业链上下游关系还没有进入核心得分。把这些限制写清楚，可以避免把系统原型解释为已经完成充分投资验证的生产工具。",
        "安全性和合规性在本课题中主要表现为边界控制。系统不处理真实交易指令，也不提供自动下单能力；论文中的结果只用于研究展示，不构成投资建议。这一边界需要在论文解释中保持明确，避免把毕业设计原型描述成生产级投资系统。":
            "安全性和合规性主要体现在使用边界上。系统只整理行业研究线索，不接收真实账户信息，不生成交易指令，也不提供自动下单能力；论文中的排序和案例仅用于毕业设计展示与方法说明，不构成投资建议。该边界在摘要、测试和结论中保持一致，避免把本地研究原型误写成生产级投资系统。",
        "动量因子的解释相对直观，它反映行业在当前统计窗口内是否走强。资金因子强调市场是否有持续承接，避免只有价格上涨却缺少成交和资金支持的行业被过度高估。活跃度因子用于判断该行业是否有足够交易参与，波动率代理则用于约束短期剧烈波动带来的噪声。":
            "四个行业因子分别承担不同解释任务：动量因子观察统计窗口内的价格强弱，资金因子检查上涨背后是否有承接，活跃度因子衡量交易参与程度，波动率代理则抑制短期剧烈波动带来的噪声。它们共同作用后，系统才会生成用于热力图展示的 total_score。",
        "从市场确认角度看，热点个股和核心资产往往并不重合。短期资金关注可能迅速推高某些股票的热度，但这种热度未必意味着其长期代表性更强。项目中将榜单组织为 core 和 hot 两类，正是为了保留这两种不同语义。":
            "从市场确认角度看，热点个股和核心资产并不总是重合。短期资金可能迅速推高某些股票的关注度，但这种关注并不必然代表长期行业地位。系统将输出拆为 core 和 hot 两类榜单，就是为了让综合代表性和短期冲击信号分别呈现。",
        "响应封装阶段同样重要。前端不应直接面对不同数据源的原始返回，而应接收结构稳定的字段，例如 total_score、change_pct、moneyFlow、turnoverRate、leaders、trend 和 explanation。这样前端组件才能稳定复用，也便于测试脚本检查接口契约。":
            "响应封装阶段承担接口契约作用。不同数据源的原始字段名称和单位并不一致，前端如果直接处理这些差异，页面逻辑会迅速膨胀。因此，后端将结果统一整理为 total_score、change_pct、moneyFlow、turnoverRate、leaders、trend 和 explanation 等字段，既便于组件复用，也便于测试脚本检查接口结构。",
        "适配器还承担了“尽量不让页面出现空白或不可用状态”的职责。当主源短暂不可用时，系统会尝试调用补充数据源或过期缓存；当某些行业成分股无法完整获取时，系统也会返回降级结果，并在后续详情分析中继续补充。这种策略不能保证每次都获得最完整数据，但能提高研究页面的连续性。":
            "适配器还负责控制页面降级方式。主源短暂不可用时，系统会尝试补充数据源或过期缓存；部分行业成分股暂时无法完整获取时，系统先返回降级结果，并在详情分析阶段继续补齐。该策略不能保证每次都获得最完整数据，但可以避免研究页面直接变为空白。",
        "详情弹窗则负责解释。行业详情展示趋势、成分股和覆盖信息，龙头股详情展示得分拆解、价格走势和财务字段。这样，使用者从热力图进入详情后，能沿着同一条数据链路继续追问，而不是被迫切换到另一套工具。":
            "详情弹窗承担结果追溯功能。行业详情给出趋势、成分股和覆盖信息，龙头股详情给出得分拆解、价格走势和财务字段。使用者从热力图进入详情后，可以沿着同一条数据链路继续检查，而不需要切换到另一套工具重新整理材料。",
        "龙头股双榜单样例也说明， core 与 hot 的结果并不完全相同。core 更接近行业代表性公司筛选， hot 更接近短期冲击信号筛选。两者同时展示，可以帮助使用者区分“长期值得研究的公司”和“短期市场关注度快速上升的公司”。":
            "龙头股双榜单样例表明，core 与 hot 的结果并不完全重合。core 偏向行业代表性公司筛选，hot 偏向短期冲击信号筛选。两者同时展示，使论文能够分别讨论稳定代表性和短期关注度，而不是把所有候选股票混在同一排名中解释。",
        "另外，论文能够完成，也离不开 Python、FastAPI、React、AKShare、scikit-learn 等开源工具在开发阶段提供的支持。正是因为这些工具较为成熟，本人得以将更多精力放在行业分析流程、页面交互和论文整理本身；也感谢学校和学院为毕业设计提供的学习环境与资源支持。":
            "此外，Python、FastAPI、React、AKShare、scikit-learn 等开源工具为系统开发和论文整理提供了重要支持。它们降低了数据处理、接口封装和页面实现的基础成本，使本人能够把更多精力放在行业分析流程、页面交互验证和论文材料复核上。也感谢学校和学院为毕业设计提供的学习环境与资源支持。",
        "在结果解释中，系统不足需要和测试结论一起呈现。表 6.3 从权重、数据、文本信息、投资验证和部署形态五个方面列出了当前边界及后续改进方向。":
            "测试结果只能说明系统主链路已经跑通，不能说明所有研究问题都已解决。表 6.3 将仍需保留的边界逐项列出，便于把“已经完成的系统功能”和“后续仍需研究的部分”区分开。",
        "对照任务书要求，如表 6.4 所示，本文在理论分析、模型设计和系统实现三个层面均完成了核心目标。":
            "任务书中的要求可以回到论文章节和系统证据逐项核对。表 6.4 列出了每项要求对应的论文位置、实现证据和完成状态。",
        "本文没有将研究内容组织为若干相互割裂的模块，而是按照系统实际使用顺序进行梳理。较符合常规行业研究场景的路径是：首先进入行业页面，观察热力图和排行榜呈现的横截面结果；随后围绕某一行业继续查看趋势、成分股和龙头股详情；最后再解释这些结果由适配器、缓存和评分器逐步生成的过程。":
            "本文的研究内容按照系统证据链展开，而不是拆成互不关联的功能说明。使用者首先在行业页面观察热力图和排行榜，得到样本窗口内的横截面线索；随后进入某一行业的趋势、成分股和龙头股详情，检查热点是否具有内部支撑；最后再回到 THS-first 适配、缓存回退、IndustryAnalyzer 评分和 LeaderStockScorer 遴选过程，说明页面结果如何由后端计算得到。",
        "动量研究为本文提供了最直接的理论支撑。Jegadeesh 和 Titman 证明了股票层面的中期动量现象，Moskowitz 和 Grinblatt 则进一步讨论了行业动量。[13][15] 这些研究说明，行业层面的相对强弱具有研究意义，也支持本文按 1 日、5 日、10 日等窗口观察行业状态。":
            "本文引用动量研究并不是为了直接构造交易策略，而是为行业横截面观察提供理论依据。Jegadeesh 和 Titman 讨论了股票层面的中期动量现象，Moskowitz 和 Grinblatt 进一步从行业层面解释了相对强弱的延续性。[13][15] 因此，本系统在行业热度模块中保留 1 日、5 日和 10 日窗口，用于比较短期冲击与稍长周期强弱是否一致。",
        "第三类需求是龙头股解释。系统不仅要返回候选股票，还要展示为什么这些股票排在前面，包括市值、估值、盈利、成长、动量和活跃度等维度。这样既便于使用者判断结果是否可信，也方便论文把模型设计和页面展示对应起来。":
            "第三类需求是龙头股结果解释。页面不能只给出股票代码和排名，还需要展示候选股票在市值、估值、盈利、成长、动量和活跃度上的具体贡献。这样既能让使用者检查某只股票进入 core 或 hot 榜单的原因，也能让论文把公式（4.4.1）的六维权重与图 5.3 的详情页展示对应起来。",
        "如图 3.1 所示，系统总体上采用四层结构：表现层、服务层、分析层和数据层。这样拆分不是为了把架构图画得更复杂，而是因为行业研究页面从请求发出到结果落地，确实会依次经过这四层。表现层主要是前端行业主页面、热力图、排行榜以及行业详情和龙头股详情弹窗，回放、观察列表和偏好配置等功能则作为辅助研究交互存在；服务层由 FastAPI 行业接口构成，负责请求接收、参数校验、缓存与响应封装；分析层由行业分析器和龙头股评分器组成，承担行业综合评分、龙头股筛选和评分拆解；数据层则以 THS-first 适配器为核心，统一协调同花顺、AKShare、新浪和腾讯等数据源。":
            "如图 3.1 所示，系统采用表现层、服务层、分析层和数据层四层结构。该划分对应行业热度页面的真实运行路径：表现层承载热力图、排行榜、详情弹窗、回放和偏好配置；服务层通过 FastAPI 行业接口完成参数校验、缓存命中和响应封装；分析层由 IndustryAnalyzer 与 LeaderStockScorer 组织行业评分、个股筛选和维度拆解；数据层以 THS-first 适配为主线，并在必要时调用 AKShare、新浪和腾讯等补充来源。通过这种结构，论文中的架构图可以直接对应系统中的接口、类和页面状态。",
        "从后续扩展看，若要将行业信号进一步扩展为回测验证或研究报告生成流程，可以把固定快照转成更规范的数据表，再与后续研究记录关联。这样能够形成从行业发现、样例保存到策略验证的更完整闭环，但这已经属于当前课题之后的工作。":
            "后续若继续扩展，可以把当前保存的热力图快照整理为按日期、行业和窗口划分的结构化样本，再与换仓规则、持有期收益和研究备注关联。这样可以从毕业设计阶段的固定样例分析推进到滚动回测或研究报告生成，但这部分需要更长样本和交易成本假设，已超出本文当前验证范围。",
        "动量和活跃度维度用于补充市场确认。即使一家企业基本面较好，如果短期几乎没有成交和关注，也未必适合作为当前热点行业中的候选；反过来，只有动量而缺少基本面支撑的股票，则更适合放在 hot 观察榜中继续跟踪。":
            "在六维龙头股评分中，动量和活跃度主要承担市场确认作用。若一家企业的规模、估值和盈利指标较好，但样本窗口内成交与关注度不足，系统不会简单把它解释为当前热点行业的代表；若某只股票短期动量较强而基本面支撑不足，则更适合进入 hot 观察榜，用于提示短期冲击而非直接替代 core 榜单。",
        "从工程实现看，公式（4.4.1）对应的是完整评分链路下的六维加权总分，其中 s1 至 s6 分别表示市值规模、估值水平、盈利能力、成长性、价格动量和交易活跃度的归一化得分。完整评分链路会在统一的 raw_data 结构上聚合估值、财务和行情字段，并按该权重体系生成总分。因此，前端 core 榜单主要反映这一综合评分语义；hot 榜单则更强调短期涨幅与资金承接。":
            "公式（4.4.1）在代码中对应完整龙头股评分链路。s1 至 s6 分别承接规模、估值、盈利、成长、动量和活跃度字段，系统会先把这些字段归一化，再按照权重生成综合分。core 榜单主要使用该综合分解释行业代表性，hot 榜单则保留短期涨幅和资金承接信号，两者共同构成详情页中的双路径候选结果。",
        "盈利和成长维度更接近基本面判断。ROE、营收同比和利润同比能够反映公司经营质量，但这些字段更新频率低、接口覆盖并不总是完整。因此完整评分链路依赖缓存补充，快速评分链路则在必要时采用中性值，保证页面候选列表不会因个别字段缺失而中断。":
            "盈利与成长字段主要用于补充基本面解释。系统优先读取 ROE、营业收入同比和利润同比等指标，但这些字段受披露频率和数据源覆盖影响，不能保证每次请求都完整返回。因此，完整评分链路会优先使用缓存补齐财务字段；快速评分链路则在字段缺失时采用中性处理，使候选列表能够先展示，再由详情页继续补充解释。",
        "在代码组织上，适配层承担了大量细节性但关键的工作。例如行业节点需要映射，股票名称需要转换成代码，部分字段需要从字符串转成数值，某些接口失败后还要尝试代理节点或缓存结果。若这些工作分散到前端或评分器中，系统维护成本会明显上升。":
            "在代码组织上，适配层集中处理最容易出错的数据细节。行业节点映射、股票名称转代码、字符串数值化、接口失败后的代理节点尝试以及缓存结果读取，都在数据层附近完成。这样做可以让前端只面对统一字段，让 IndustryAnalyzer 和 LeaderStockScorer 专注评分逻辑，避免数据源差异向页面和模型层扩散。",
        "后端接口设计还需要考虑前端调用顺序。行业页通常会并行请求热力图、排行榜和龙头股摘要，如果接口返回结构不稳定，前端组件就需要写大量兼容逻辑。因此，后端尽量把字段整理成统一响应，让前端专注展示和交互。":
            "后端接口按照前端行业页面的调用顺序设计。首屏通常同时请求热力图、热门行业列表和龙头股摘要，下钻时再请求行业详情、趋势和个股拆解。若这些接口字段口径不一致，前端就需要堆叠大量兼容逻辑；因此，服务层统一整理 total_score、leaders、trend、explanation 等字段，使页面组件主要负责展示和交互。",
        "接口缓存的粒度也需要区分。热门行业列表和热力图适合短时间缓存，避免使用者切换视图时重复计算；行业详情和龙头股详情则需要根据行业名称、统计窗口和榜单类型区分缓存键。这样的设计能在性能和结果准确性之间取得平衡。":
            "接口缓存按数据成本和交互频率区分。热门行业列表与热力图属于首屏高频结果，适合短时间复用；行业详情和龙头股详情依赖行业名称、统计窗口和榜单类型，需要使用更细的缓存键；偏好配置则要求保存后稳定读取。这样的划分比统一缓存时间更贴近页面实际使用方式，也能减少外部数据源波动对演示过程的影响。",
        "从交互设计看，热力图负责快速建立市场全局印象。颜色维度使使用者看到强弱，尺寸维度帮助使用者理解市值或成交背景，统计窗口则支持比较短期和稍长周期的变化。对于行业研究而言，这比单纯表格更容易发现结构性热点。":
            "从交互设计看，热力图并不是把排行榜换成彩色图形，而是把强弱、规模背景和统计窗口放在同一屏中。颜色用于表达行业热度相对强弱，面积用于辅助理解市值或成交背景，窗口切换则用于比较短期冲击与稍长周期表现。对行业研究而言，这种视图能够更快暴露板块之间的结构差异，再由详情弹窗承接后续解释。",
        "龙头股双榜单样例也说明，core 与 hot 的结果并不完全相同。core 更接近行业代表性公司筛选，hot 更接近短期冲击信号筛选。两者同时展示，可以帮助使用者区分“长期值得研究的公司”和“短期市场关注度快速上升的公司”。":
            "表 6.2 中的龙头股双榜单显示，core 与 hot 的候选并不完全重合。core 更强调行业代表性、盈利质量和规模基础，hot 更强调统计窗口内的价格冲击和市场活跃度。将两类结果并列展示，可以避免把短期关注度直接等同于长期行业地位，也让第六章能够分别解释稳定代表性和阶段性热点。",
        "模型部分没有额外引入难以解释的黑箱方法，而是围绕本系统中的两个核心分析类展开。IndustryAnalyzer 负责判断行业热度，主要依据价格变化、资金承接、交易活跃度和波动率代理；LeaderStockScorer 负责在行业内部比较个股，把规模、估值、盈利、成长、动量和活跃度六个维度合成综合分，并保留快照场景下的快速评分链路。这种处理方式没有追求模型复杂度，但更容易与系统实现一一对应。":
            "模型设计围绕代码中实际承担计算任务的两个类展开。IndustryAnalyzer 处理行业横截面，把涨跌、资金、成交活跃和波动代理整理为行业热度；LeaderStockScorer 进入行业内部，把规模、估值、盈利、成长、动量和活跃度映射为候选股票分数，并在固定快照场景下提供快速路径。本文因此强调可解释和可复核，而不是引入难以回到代码定位的黑箱模型。",
        "从工程角度看，这项工作的价值不仅在于实现一个展示页面。一方面，系统将多源金融数据整理为可比较的行业研究结果，能够降低人工筛选成本；另一方面，评分过程保持较强可解释性，便于将工程实现、模型口径和样例结果统一说明。从项目完成度看，该原型已经能够支撑毕业设计展示、运行截图获取和后续迭代，而不是停留在方案层面。":
            "从工程角度看，本课题的关键不只是完成一个页面，而是让行业目录、资金流、成分股、估值和财务字段能够在同一条链路中被清洗、评分和展示。评分结果能够回到字段和权重说明，截图样例也能回到保存的热力图快照与龙头股案例，因此论文中的模型、页面和数据证据可以相互核对。",
        "综合相关文献与本系统实现可以发现，能够长期代表行业状态的股票，通常不能只依据单日涨幅判断。规模、盈利、成长和估值提供的是相对稳定的基本面线索，成交额、换手率和近期动量则更侧重反映市场对该判断的短期确认程度。[4-5][8-12]":
            "综合企业估值、多因子选股和本系统实现可以看出，行业代表性不能由单日涨幅单独决定。市值和盈利质量更接近长期代表性，估值和成长性用于补充基本面解释，成交额、换手率和近期动量则反映样本窗口内的市场确认。[4-5][8-12]",
        "从市场确认角度看，热点个股和核心资产并不总是重合。短期资金可能迅速推高某些股票的关注度，但这种关注并不必然代表长期行业地位。系统将输出拆为 core 和 hot 两类榜单，就是为了让综合代表性和短期冲击信号分别呈现。":
            "热点候选与核心候选在含义上需要分开。短期资金承接会让某些个股进入 hot 榜单，但这类信号只能说明阶段性关注度上升；core 榜单则更依赖规模、估值、盈利和成长字段。系统保留两张榜单，是为了避免把短期活跃误读为长期行业地位。",
        "因此，龙头股评价应当同时考虑“稳定代表性”和“短期市场确认”。前者更依赖市值、估值、盈利和成长，后者更依赖涨跌幅、成交额、换手率和资金承接。本文的评分模型并不宣称能给出唯一正确答案，而是为使用者提供可解释的排序依据。":
            "因此，龙头股评价被拆成两类证据：一类是市值、估值、ROE 和成长指标所反映的稳定代表性，另一类是涨跌幅、成交额、换手率和资金承接所反映的短期市场确认。本文的目标是给出可追溯的排序依据，而不是把评分结果写成唯一正确的投资结论。",
        "本文把行业热点理解为三个信号的交叉结果：首先，行业收益中可能包含共同风险暴露和风格因子影响；其次，行业动量研究说明强势板块在短窗口内可能延续资金关注；最后，行为金融研究提示短期热点也可能混入情绪放大和过度反应。[8-20] 因此，系统没有把热门行业简化为涨幅榜，而是要求价格、资金、活跃度和风险约束共同参与判断。":
            "本文没有把行业热点直接等同于涨幅榜。行业收益可能受到共同风险暴露和风格因子影响，行业动量又提示强势板块在短窗口内可能延续，行为金融研究则提醒短期热点中可能包含情绪放大。[8-20] 因此，IndustryAnalyzer 同时读取价格、资金、活跃度和波动代理，让行业热度来自多维证据而非单项排序。",
        "围绕行业热度识别这一主线，系统采用的技术可以分为四组。第一组是 pandas、NumPy 和 SciPy，用于把行业摘要、资金流、成分股和财务字段整理成可比较的横截面数据；第二组是 scikit-learn 与 Matplotlib，用于辅助聚类解释和图表生成；第三组是 FastAPI，用于把分析结果封装为前端可调用的接口；第四组是 React 与 Ant Design，用于承载热力图、排行榜和详情弹窗。[6-7][34-37]":
            "围绕行业热度主线，技术选型直接对应数据处理和页面展示。pandas、NumPy 与 SciPy 负责清洗行业摘要、资金流、成分股和财务字段；scikit-learn 用于聚类辅助解释，Matplotlib 用于生成论文材料中的图表；FastAPI 将分析结果封装为接口；React 与 Ant Design 承载热力图、排行榜和详情弹窗。[6-7][34-37]",
        "在算法层面，系统主要使用加权评分和 K-Means 聚类。K-Means 本身是一种经典聚类方法，后续研究又提出了 k-means++ 初始化和轮廓系数评估等改进思路。[30-33] 在本文里，聚类不是为了替代行业得分，而是用于观察若干行业是否形成相近热点簇，属于辅助解释工具。":
            "算法部分以加权评分为主，K-Means 只承担辅助观察作用。k-means++ 初始化和轮廓系数等方法能够改善聚类稳定性与评价方式，[30-33] 但本文并不让聚类结果替代热度分数，而是把它作为判断若干行业是否呈现相近特征的补充视角。",
        "在后端服务层，FastAPI 提供了较轻量的接口组织方式，适合把 Python 分析能力封装成前端可调用的 REST 接口。[38-39] REST 架构思想强调资源、统一接口和无状态交互，这与本系统中热门行业、热力图、趋势、龙头股详情等接口设计具有一致性。":
            "后端服务层使用 FastAPI，是因为行业热度、热力图、趋势、龙头股详情和偏好配置都可以被组织为清晰的 REST 资源。[38-39] 前端只需要按照接口契约读取结果，Python 分析层则继续保留在后端，二者之间通过统一响应字段衔接。",
        "安全性和合规性主要体现在使用边界上。系统只整理行业研究线索，不接收真实账户信息，不生成交易指令，也不提供自动下单能力；论文中的排序和案例仅用于毕业设计展示与方法说明，不构成投资建议。该边界在摘要、测试和结论中保持一致，避免把本地研究原型误写成生产级投资系统。":
            "安全与合规边界在论文中保持收紧：系统不读取真实账户，不接收交易委托，也不会生成自动下单指令。行业排序和龙头股案例只用于解释数据处理与评分方法，不构成投资建议。这一表述贯穿摘要、测试和结论，避免把本地毕业设计原型描述成生产投资系统。",
        "如图 3.1 所示，系统采用表现层、服务层、分析层和数据层四层结构。该划分对应行业热度页面的真实运行路径：表现层承载热力图、排行榜、详情弹窗、回放和偏好配置；服务层通过 FastAPI 行业接口完成参数校验、缓存命中和响应封装；分析层由 IndustryAnalyzer 与 LeaderStockScorer 组织行业评分、个股筛选和维度拆解；数据层以 THS-first 适配为主线，并在必要时调用 AKShare、新浪和腾讯等补充来源。通过这种结构，论文中的架构图可以直接对应系统中的接口、类和页面状态。":
            "图 3.1 展示的是一次行业页面请求在系统中的流向。前端表现层负责热力图、排行榜、详情弹窗、回放和偏好配置；FastAPI 服务层负责参数校验、缓存命中和响应封装；分析层调用 IndustryAnalyzer 与 LeaderStockScorer 生成行业分数和个股候选；数据层优先走 THS-first 适配，必要时再补充 AKShare、新浪或腾讯来源。这样的写法使架构图可以回到真实接口和类名核对。",
        "与仅依赖单一接口的数据抓取脚本相比，这套架构更适合支撑准实时行业研究。一方面，多源适配提高了行业目录、资金流、成分股、估值和历史走势等字段的覆盖率；另一方面，缓存和分层封装使页面响应速度与结果口径更容易保持一致。对论文写作而言，这一点具有重要意义，因为只有系统本身运行足够稳定，后续获得的快照样本和案例分析才具有说服力。":
            "如果只依赖单一接口，行业目录、资金流、成分股、估值和走势字段很容易在某个环节出现缺口。本系统把多源适配、缓存和分层封装放在同一架构中处理，目的就是让首屏响应、详情下钻和固定快照保持同一口径。对论文而言，稳定的系统输出是第六章样例分析能够复核的前提。",
        "当数据进入分析层后，系统会先对行业名称和数值字段进行整理，再计算动量、资金、活跃度和波动率代理。龙头股链路则会在行业成分股基础上继续补充市值、估值、财务和成交信息，并按 core 与 hot 两类语义组织结果。":
            "进入分析层后，行业名称会先完成映射，数值字段再转为统一口径。IndustryAnalyzer 根据动量、资金、活跃度和波动代理生成行业热度；LeaderStockScorer 则在成分股基础上补充市值、估值、财务和成交字段，并把结果拆成 core 与 hot 两类语义。",
        "采用文件存储并不代表可以放任数据无限累积。热力图历史如果持续追加，会拖慢读取和论文样例定位；财务缓存若长期不刷新，ROE、估值和成长字段也可能滞后于实际披露。为避免这些问题，系统在热力图历史记录数、文件大小和缓存有效期上设置了上限，并把这些限制写入论文，作为本地研究原型阶段的工程边界。":
            "文件存储需要控制边界。热力图历史长期累积会影响读取速度，也会让论文样例定位变慢；财务缓存若不刷新，ROE、估值和成长字段可能滞后于披露数据。系统因此限制历史记录数量、文件大小和缓存有效期，并把这些限制作为本地研究原型的工程取舍写入论文。",
        "偏好配置则更接近使用者状态。观察行业、阈值、回放窗口和当前选择会随着研究习惯变化而变化，适合按 profile 保存。这样处理可以让同一台机器上的不同研究场景互不干扰，也能为后续扩展到报告生成或回测验证留下入口。":
            "偏好配置保存的是研究现场，而不是模型参数。观察行业、阈值、回放窗口和当前选择都可能随场景变化，因此按 profile 保存更合适。同一台机器可以保留不同研究状态，后续若扩展报告生成或回测验证，也能继续读取这些配置。",
        "四个行业因子分别承担不同解释任务：动量因子观察统计窗口内的价格强弱，资金因子检查上涨背后是否有承接，活跃度因子衡量交易参与程度，波动率代理则抑制短期剧烈波动带来的噪声。它们共同作用后，系统才会生成用于热力图展示的 total_score。":
            "行业热度的四个因子在代码中分工明确。动量因子读取统计窗口内的价格变化，资金因子检查上涨是否伴随承接，活跃度因子衡量交易参与，波动代理用于压低剧烈波动带来的噪声。四类信号合成后，才形成热力图使用的 total_score。",
        "分数压缩主要服务于页面解释。横截面标准化后的原始值可能出现负数，也可能被单个极端行业拉开距离；若直接用于热力图，颜色差异会显得不稳定。系统因此把得分映射到约 20 至 95 的展示区间，让排行榜和热力图更容易阅读。需要注意的是，这一处理只改变展示尺度，不改变行业之间的相对排序逻辑。":
            "分数压缩只影响展示尺度。标准化后的原始值可能为负，也可能被少数极端行业拉开距离；如果直接进入热力图，颜色映射会不稳定。系统把得分映射到约 20 至 95 区间，是为了让排行榜和热力图更容易阅读，行业相对排序逻辑并没有因此改变。",
        "在具体计算中，行业热度模型需要同时处理字段缺失和数据源波动。表 4.1 汇总了核心指标的主要字段、回退口径和解释作用。":
            "行业热度计算必须面对字段缺失和外部接口波动。表 4.1 按价格、资金、活跃度和波动代理列出字段来源与回退口径，目的是说明 total_score 在缺少部分字段时仍然如何保持可解释。",
        "公式（4.4.1）在代码中对应完整龙头股评分链路。s1 至 s6 分别承接规模、估值、盈利、成长、动量和活跃度字段，系统会先把这些字段归一化，再按照权重生成综合分。core 榜单主要使用该综合分解释行业代表性，hot 榜单则保留短期涨幅和资金承接信号，两者共同构成详情页中的双路径候选结果。":
            "公式（4.4.1）对应 LeaderStockScorer 的完整评分路径。s1 至 s6 分别接收规模、估值、盈利、成长、动量和活跃度字段，归一化后再按权重合成为综合分。core 榜单使用这一路径解释行业代表性；hot 榜单保留短期涨幅与资金承接，二者共同形成详情页中的候选股票说明。",
        "盈利与成长字段主要用于补充基本面解释。系统优先读取 ROE、营业收入同比和利润同比等指标，但这些字段受披露频率和数据源覆盖影响，不能保证每次请求都完整返回。因此，完整评分链路会优先使用缓存补齐财务字段；快速评分链路则在字段缺失时采用中性处理，使候选列表能够先展示，再由详情页继续补充解释。":
            "盈利和成长字段用于回答候选股票为什么具有基本面支撑。系统优先读取 ROE、营收同比和利润同比，但这些指标受披露频率和数据覆盖限制，不能保证每次请求都完整。完整链路先尝试用缓存补齐财务字段；快速链路在缺口处给出中性处理，让列表先返回，再由详情页补充解释。",
        "在六维龙头股评分中，动量和活跃度主要承担市场确认作用。若一家企业的规模、估值和盈利指标较好，但样本窗口内成交与关注度不足，系统不会简单把它解释为当前热点行业的代表；若某只股票短期动量较强而基本面支撑不足，则更适合进入 hot 观察榜，用于提示短期冲击而非直接替代 core 榜单。":
            "六维评分中的动量和活跃度用于检查市场是否确认基本面线索。若企业规模、估值和盈利表现尚可，但样本窗口内成交与关注不足，它未必适合作为当前热点代表；若短期动量较强但基本面字段支撑不足，则放入 hot 观察榜更合适，不直接替代 core 候选。",
        "在代码组织上，适配层集中处理最容易出错的数据细节。行业节点映射、股票名称转代码、字符串数值化、接口失败后的代理节点尝试以及缓存结果读取，都在数据层附近完成。这样做可以让前端只面对统一字段，让 IndustryAnalyzer 和 LeaderStockScorer 专注评分逻辑，避免数据源差异向页面和模型层扩散。":
            "适配层负责把外部数据源的不稳定性挡在评分逻辑之外。行业节点映射、股票名称转代码、字符串转数值、代理节点尝试和缓存读取，都在数据层附近完成；前端接收统一字段，IndustryAnalyzer 与 LeaderStockScorer 只处理已经标准化的数据，维护成本因此更可控。",
        "适配器还负责控制页面降级方式。主源短暂不可用时，系统会尝试补充数据源或过期缓存；部分行业成分股暂时无法完整获取时，系统先返回降级结果，并在详情分析阶段继续补齐。该策略不能保证每次都获得最完整数据，但可以避免研究页面直接变为空白。":
            "适配器还决定页面如何降级。主源短时不可用时，系统会尝试补充来源或读取过期缓存；成分股暂时不完整时，先返回可解释的降级结果，详情分析阶段再继续补齐。该策略不追求每次都最完整，但能避免行业页面在外部接口波动时直接空白。",
        "LeaderStockScorer 的职责并不只是生成股票得分。在同一行业内，该模块既需要输出列表排序，也需要向详情页提供评分拆解和原始字段，因此同时保留了完整评分与快照快速评分两条链路。前者更适合龙头股详情和深度研究，后者则用于在行业列表页中快速形成候选股票集合。":
            "LeaderStockScorer 在本课题中承担两类输出。列表页需要快速得到 core/hot 候选集合，详情页则需要解释每只股票的维度得分和原始字段。为兼顾这两种场景，代码中保留完整评分路径和快照快速评分路径，分别服务详情解释和首屏候选生成。",
        "模块还会把结果组织为 core 和 hot 两类榜单。core 更偏向六维综合质量筛选，hot 更强调短期涨幅和资金承接；两者共用同一套 raw_data 结构，但在无法获得 ROE 或增长字段时，快速链路会用中性分处理缺口。这样做的优势在于，页面响应速度和评分口径之间可以取得相对平衡，而不是二选一。":
            "该模块最终输出 core 与 hot 两张榜单。core 侧重六维综合质量，hot 侧重短期涨幅和资金承接；两者共用 raw_data，但快速链路在缺少 ROE 或增长字段时采用中性分。这样可以让页面先呈现可用候选，同时保留后续详情解释需要的评分口径。",
        "接口缓存按数据成本和交互频率区分。热门行业列表与热力图属于首屏高频结果，适合短时间复用；行业详情和龙头股详情依赖行业名称、统计窗口和榜单类型，需要使用更细的缓存键；偏好配置则要求保存后稳定读取。这样的划分比统一缓存时间更贴近页面实际使用方式，也能减少外部数据源波动对演示过程的影响。":
            "接口缓存按照页面访问路径设置。热门行业和热力图是首屏高频数据，可以短时间复用；行业详情和龙头股详情与行业名称、统计窗口、榜单类型绑定，需要更细的缓存键；偏好配置则强调保存后稳定读取。不同缓存粒度可以减少重复请求，也能降低外部数据源波动对演示的影响。",
        "这种拆分可以降低单次请求压力。若将所有字段集中在单一接口中，首屏加载会被详情数据拖慢；若每个细粒度字段都组织为单独接口，前端又会出现过多请求。当前设计介于两者之间，将常用汇总结果和详情结果分开，既能保证首屏速度，也能支持使用者继续下钻。":
            "接口拆分遵循首屏优先原则。热力图和排行榜只返回汇总字段，详情弹窗再请求趋势、成分股、财务和维度拆解；这样既避免一个大接口拖慢首屏，也避免过多细粒度请求增加前端复杂度。使用者可以先看到行业概况，再逐步下钻。",
        "从交互设计看，热力图并不是把排行榜换成彩色图形，而是把强弱、规模背景和统计窗口放在同一屏中。颜色用于表达行业热度相对强弱，面积用于辅助理解市值或成交背景，窗口切换则用于比较短期冲击与稍长周期表现。对行业研究而言，这种视图能够更快暴露板块之间的结构差异，再由详情弹窗承接后续解释。":
            "热力图的作用是把行业横截面先摊开。颜色显示热度强弱，面积提示市值或成交背景，窗口切换用于比较短期冲击和稍长周期表现。研究者在这一屏中发现结构差异后，再通过详情弹窗查看趋势、成分股和龙头股解释。",
        "如图 5.3 所示，龙头股详情页面展示综合得分、维度拆解、价格走势与关键财务字段，能够帮助使用者理解候选股票被遴选出的具体依据。":
            "图 5.3 展示的是龙头股详情弹窗。该页面把综合得分拆到规模、估值、盈利、成长、动量和活跃度等维度，并同时呈现走势和财务字段，便于核对某只股票进入候选榜单的具体原因。",
        "偏好配置和历史快照让前端页面具备持续研究能力。观察列表保存使用者关注的行业，回放功能帮助使用者比较不同时间点的横截面状态，本地状态则减少重复配置。这些能力虽然不是评分模型本身，却会明显影响系统是否像一个可持续使用的研究原型。":
            "偏好配置和历史快照补足了持续研究环节。观察列表保留重点行业，回放功能对比不同时间点的横截面状态，本地状态减少重复设置。它们不直接参与评分，却决定系统能否从一次性展示变成可回看的研究工具。",
        "持续跟踪功能把一次性的行业排序保存为可回看的研究材料。某个行业在当前窗口排在前列，并不说明后续仍会维持强势；只有保存历史快照，才能比较其后续是短暂冲高、连续走强，还是在资金流和活跃度上出现背离。":
            "持续跟踪功能把单次行业排序沉淀为历史材料。某个行业在当前窗口靠前，只说明样本时点较强；保存快照后，后续才能继续比较它是短暂冲高、连续走强，还是在资金流和活跃度上出现背离。",
        "从固定快照看，能源金属、元件、军工电子和小金属等方向在样本窗口内排序靠前，说明系统能够捕捉阶段性强势板块。不过，这一结论只对应保存快照的时点；如果市场环境、政策预期、行业景气度或资金风格发生变化，排序结果也会随之调整。":
            "以保存的五日窗口快照为例，能源金属、元件、军工电子和小金属等行业位于前列，说明系统在该时点捕捉到了阶段性强势方向。该结论只绑定这组快照；市场环境、政策预期、景气度或资金风格变化后，排序也会重新变化。",
        "表 6.2 中的龙头股双榜单显示，core 与 hot 的候选并不完全重合。core 更强调行业代表性、盈利质量和规模基础，hot 更强调统计窗口内的价格冲击和市场活跃度。将两类结果并列展示，可以避免把短期关注度直接等同于长期行业地位，也让第六章能够分别解释稳定代表性和阶段性热点。":
            "表 6.2 需要按两种语义阅读。core 候选更看重行业代表性、盈利质量和规模基础，hot 候选更看重样本窗口内的价格冲击和市场活跃。两类结果并列后，第六章可以分别讨论稳定代表性与阶段性热点，而不把短期活跃直接写成长期龙头。",
        "固定快照的意义在于将动态市场状态固化为可复核样本。若论文每次生成都重新获取最新行情，表 6.2 的行业排序会不断变化，评阅者和读者将难以复核。使用固定快照后，样例分析可以围绕同一时点展开，结论也不会随着行情刷新而漂移。":
            "固定快照把动态行情保存成可复核样本。若每次生成论文都重新获取行情，表 6.2 的行业排序会随市场变化而变化；使用固定快照后，样例分析始终围绕同一时点展开，评阅时也可以回到相同数据检查。",
        "从当前仓库实际保留的行业模块来看，其主要价值在于同一套数据入口已经能够支撑从热力图到个股详情的连续分析。使用者先在页面中查看行业排序，再进入趋势、龙头股和评分拆解时，背后仍然使用同一条适配、评分和缓存链路，因此结果解释具有较好的连贯性。":
            "从当前行业模块看，系统价值体现在连续分析链路上。热力图、趋势、龙头股和评分拆解都来自同一套适配、评分和缓存流程；页面从行业排序进入个股详情时，解释口径不会突然切换。这种连贯性是本课题能够支撑论文样例分析的基础。",
        "第三个特征是工程上保留了现实妥协。接口回退、过期缓存、中性值、快照保存和偏好持久化都不是理论模型里的核心概念，但它们决定了系统能否在真实数据环境中持续运行。":
            "第三个特征是保留工程妥协。接口回退、过期缓存、中性值、快照保存和偏好持久化虽然不是评分公式的一部分，却决定系统能否在外部数据源波动时继续给出可解释结果。",
        "但系统边界也比较明确。权重目前仍以工程经验为主，部分行业在成分股映射不完整时仍会进入降级路径；模块刷新主要依赖 REST、缓存和历史快照，并不是持续流式的实时系统；行业识别结果与后续收益之间的量化验证也仍需进一步补强。这些问题不会影响毕业设计展示，但属于后续迭代中需要继续优化的重点。":
            "系统边界同样需要说明。权重仍来自文献口径、工程经验和样例调试，成分股映射不完整时会进入降级路径；模块刷新依赖 REST、缓存和历史快照，并非持续流式系统；行业识别结果与后续收益之间还缺少长周期回测。这些问题不影响毕业设计展示，但属于后续研究应继续补强的部分。",
        "此外，Python、FastAPI、React、AKShare、scikit-learn 等开源工具为系统开发和论文整理提供了重要支持。它们降低了数据处理、接口封装和页面实现的基础成本，使本人能够把更多精力放在行业分析流程、页面交互验证和论文材料复核上。也感谢学校和学院为毕业设计提供的学习环境与资源支持。":
            "此外，本课题开发与论文整理过程中使用了 Python、FastAPI、React、AKShare、scikit-learn 等开源工具。它们为数据清洗、接口封装、页面实现和辅助分析提供了基础支持，使本人能够将主要精力放在行业热度流程、页面验证和论文材料核对上。也感谢学校和学院为毕业设计提供的学习环境与资源支持。",
        "过去，这类工作较多依赖研究人员切换行情终端、阅读行业报告和积累经验。在市场节奏较慢时，该方式尚可满足部分研究需求；但随着市场数据规模和更新频率提高，仅依赖人工方式已经难以同时完成横截面比较、历史跟踪和多维交叉验证。因此，将大数据处理、量化评分和可视化展示连接为可持续运行的研究链路，相比单纯提出纸面分析方法更具有工程实践意义。[1-2]":
            "传统行业研究往往需要在行情终端、行业资料和经验判断之间来回切换。数据更新频率提高后，人工方式很难同时完成横截面比较、历史回看和多维验证。本文因此把数据处理、量化评分和可视化展示接成一条可运行链路，使行业热度识别不只停留在文字方案上。[1-2]",
        "行业动量研究进一步说明，行业层面的信息扩散和资金再配置具有一定持续性。Moskowitz 和 Grinblatt 对行业动量的研究表明，行业因素能够解释相当一部分动量收益；Hou 关于行业信息扩散的研究也指出，行业之间可能存在领先和滞后关系。[13-16] 这类研究给本文的启发是：如果系统能稳定保留行业横截面结果，就可以为后续复盘行业轮动提供材料。":
            "行业动量文献说明，行业层面的强弱变化并非完全随机。Moskowitz 和 Grinblatt 讨论了行业因素与动量收益之间的关系，Hou 的研究则提示行业间可能存在信息传导和领先滞后。[13-16] 这些结论启发本文保留行业横截面快照，为后续复盘行业轮动提供可回看的材料。",
        "近年机器学习方法已经被广泛用于资产定价和金融时间序列研究，相关文献说明复杂模型能够在大量特征中捕捉非线性结构，但模型效果往往受样本划分、交易成本、数据窥探和样本外检验影响。[19][21-23] 因此，本文没有把系统定位为收益预测器，而是定位为行业研究辅助工具：先把热点横截面和候选龙头股解释清楚，再为后续更严格的回测留下接口。":
            "机器学习方法已广泛用于资产定价和金融时间序列研究，但相关效果通常依赖样本划分、交易成本设置、数据窥探控制和样本外检验。[19][21-23] 本文没有把系统写成收益预测器，而是定位为行业研究辅助工具：先解释热点横截面和候选龙头股，再为以后更严格的回测保留数据入口。",
        "在分析方法上，本文采用文献梳理、系统分析、模型归纳、工程实现说明和样例验证相结合的方式。文献梳理用于确认行业轮动、多因子筛选和金融大数据处理的研究基础；系统分析用于界定需求和架构；模型归纳用于解释评分逻辑；样例验证则使用系统保存的热力图快照和龙头股样本说明系统输出。":
            "本文的研究方法分为四步：先梳理行业轮动、多因子筛选和金融大数据处理文献，再对系统需求与架构进行分析；随后把行业热度和龙头股评分抽象为可解释模型；最后使用已保存的热力图快照和龙头股样本验证输出结果。",
        "行业信息扩散研究则解释了为什么行业内部和行业之间可能存在领先滞后关系。Hou 的研究表明，行业信息可能通过上下游或相关行业逐步反映到股票价格中。[16] 因此，系统只展示某一行业当天涨幅是不够的，还需要保留趋势、成分股和龙头股详情，使使用者继续沿行业线索下钻。":
            "行业信息扩散研究解释了行业间可能出现领先滞后关系的原因。Hou 指出，信息可能沿上下游或相关行业逐步反映到价格中。[16] 因此，本系统除当天行业涨跌外，还保留趋势、成分股和龙头股详情，便于沿行业线索继续追踪。",
        "根据上述需求分析，行业热度分析系统的核心功能可整理为表 3.1。该表把使用者操作、系统处理和输出结果对应起来，便于后续设计和测试逐项核对。":
            "根据上述需求，表 3.1 将行业热度分析系统拆成数据采集、行业评分、龙头股遴选、可视化展示和偏好保存五类功能。每一类都对应后续章节中的实现位置和测试依据。",
        "另一个需要考虑的问题是外部接口稳定性。行业目录、资金流和个股字段只要有一个来源出现短时波动，页面就可能出现空值或局部缺口，因此系统必须允许主路径失败后继续回退，并让热力图、排行榜和详情页尽量保持可用。后续若继续接入新的指标或研究视图，系统结构也应避免每次扩展都进行大范围改造，因此可扩展性同样是非功能需求的一部分。":
            "外部接口稳定性也是非功能需求。行业目录、资金流或个股字段任一来源短时波动，都可能造成页面局部缺口；因此系统需要主路径失败后的回退、缓存和降级说明。后续若增加指标或研究视图，也应尽量复用现有适配层和接口结构，避免大范围改造。",
        "响应封装阶段承担接口契约作用。不同数据源的原始字段名称和单位并不一致，前端如果直接处理这些差异，页面逻辑会迅速膨胀。因此，后端将结果统一整理为 total_score、change_pct、moneyFlow、turnoverRate、leaders、trend 和 explanation 等字段，既便于组件复用，也便于测试脚本检查接口结构。":
            "响应封装相当于前后端之间的契约。不同数据源给出的字段名、单位和缺失方式并不一致，若全部交给前端处理，页面逻辑会变得松散。后端统一输出 total_score、change_pct、moneyFlow、turnoverRate、leaders、trend 和 explanation 等字段，测试脚本也可以按这些字段检查接口结构。",
        "如果未来系统要走向多人协作或在线部署，存储层可以进一步迁移到数据库、对象存储或消息队列体系。但在当前本科毕业设计范围内，文件快照加浏览器状态已经能够满足样例复核、页面回放和偏好保存三类核心需求。":
            "若未来支持多人协作或在线部署，存储层可以迁移到数据库、对象存储或消息队列。但在当前本科毕业设计范围内，文件快照和浏览器状态已经能够覆盖样例复核、页面回放与偏好保存三项核心需求。",
        "这种存储选择反映了毕业设计阶段的边界：行业热度核心链路追求的是可运行、可回看和可解释，而不是一开始就搭建复杂数据平台。只要热力图历史、偏好配置和财务缓存能够稳定保存，系统就已经具备支持论文样例复核的基础。":
            "这种存储选择体现了课题边界：当前重点是让行业热度链路可运行、可回看、可解释，而不是先搭建复杂数据平台。热力图历史、偏好配置和财务缓存能够稳定保存后，论文样例复核所需的基础证据就已经具备。",
        "根据系统中的实际实现，行业热度判断主要围绕四类信号展开：价格动量、资金流强度、交易活跃度和行业波动率。实际代码并没有停留在抽象的“四因子加总”上，而是把每个信号都落到了具体字段和回退口径上。动量主要来自 change_pct 或 weighted_change，资金因子对应 flow_strength，活跃度优先看 avg_volume，无法获得时再退化到 turnover_rate。波动率则优先尝试行业指数历史收益率，无法获得时退回振幅、换手率或涨跌幅代理值。这种写法更符合行业轮动研究里强调多维信号综合判断的思路。[3][13-20]":
            "在代码实现中，行业热度由价格动量、资金流强度、交易活跃度和行业波动率四类信号构成。动量读取 change_pct 或 weighted_change，资金因子读取 flow_strength，活跃度优先采用 avg_volume，缺失时退回 turnover_rate；波动率优先使用行业指数历史收益率，无法获取时使用振幅、换手率或涨跌幅代理。该设计把多维判断落到具体字段和回退路径上。[3][13-20]",
        "该公式中的权重与系统代码中的默认权重保持一致。公式（4.2.1）对应的是横截面原始评分，并不直接等同于前端最终展示值。系统在完成加权之后，会进一步调用分数压缩函数，把结果统一映射到约 20 至 95 的展示区间，以减少样本集中时 0 分和 100 分贴边的问题。因此，排行榜和热力图中的 total_score 更适合用于相对比较，而不应被理解为行业的绝对评价值。":
            "公式（4.2.1）的权重与代码默认值一致，用于生成行业横截面原始评分。前端展示前还会经过分数压缩，把结果映射到约 20 至 95 区间，避免样本集中时出现 0 分或 100 分贴边。排行榜和热力图中的 total_score 因此只适合做相对比较，不应理解为行业绝对价值判断。",
        "模型权重目前仍属于经验型设置。本文选择动量与资金流权重较高，是因为行业热点通常同时需要价格表现和资金确认；活跃度与波动率权重较低，是因为它们更接近辅助和约束信号。后续若要增强严谨性，可以基于历史样本进行权重敏感性分析和滚动回测。[19-23]":
            "模型权重仍属于经验型设置。动量和资金流权重较高，是因为热点行业通常需要价格表现与资金承接同时出现；活跃度和波动率权重较低，是因为二者更多承担辅助和约束作用。后续可基于历史样本做敏感性分析和滚动回测。[19-23]",
        "完整评分链路的意义在于解释，而不仅是排序。前端详情页展示维度拆解后，使用者可以看到某只股票是因为规模、盈利、成长还是短期动量获得较高得分，这比单一总分更符合研究场景。与复杂机器学习选股模型相比，本文的六维评分明显更简单，但它的优势是透明，适合本科毕业设计阶段的展示和复核。[21-23]":
            "完整评分链路的重点是解释排序来源。详情页展示维度拆解后，可以看到候选股票究竟受规模、盈利、成长还是短期动量影响。与复杂机器学习选股模型相比，六维评分更简单，但字段来源和权重更容易复核，也更符合本科毕业设计的展示要求。[21-23]",
        "这种双榜单设计是本文比较重要的工程取舍。core 和 hot 不是重复列表，而是分别回答两个问题：前者回答“谁更接近行业核心代表”，后者回答“谁在短期内获得更高市场关注”。把二者同时展示，可以减少使用者将短期涨幅等同于长期龙头的误解。":
            "core 与 hot 不是重复列表。core 回答“谁更接近行业核心代表”，hot 回答“谁在短期内获得更高市场关注”。二者同时展示，可以把长期代表性和短期活跃度拆开解释，减少把涨幅直接等同于龙头地位的误解。",
        "完整评分链路会先补齐估值、盈利、成长和行情字段，再按六维权重计算综合得分。该链路主要服务详情页：当某只股票进入 core 榜单后，页面需要说明其优势来自规模、估值、ROE、成长性还是短期动量，而不是只给出一个排序数字。":
            "完整评分链路先补齐估值、盈利、成长和行情字段，再按六维权重计算综合得分。它主要服务详情页：某只股票进入 core 榜单后，页面需要说明优势来自规模、估值、ROE、成长性还是短期动量，而不是只给出排名。",
        "在快速评分链路中，系统更关注候选集合的快速形成。hot 榜单强调短期涨幅和资金承接，core 榜单强调相对稳定的综合质量。两类榜单共同存在，使系统既能回应热点交易线索，也能保留行业代表性筛选逻辑。":
            "快速评分链路更关注候选集合形成速度。hot 榜单突出短期涨幅和资金承接，core 榜单保留相对稳定的综合质量；两者并行后，页面既能提示短期热点，也能保留行业代表性筛选逻辑。",
        "后端接口层在系统中承担的是“把分析能力组织成可稳定调用的服务”。热门行业列表、成分股、热力图、热力图历史、行业趋势、龙头股列表、龙头股详情和偏好配置这些接口，基本把前端行业工作区需要的数据都覆盖了；聚类和轮动接口则更多承担辅助分析作用。[1-2][6-7]":
            "后端接口层把分析能力整理成前端可稳定调用的服务。热门行业、成分股、热力图、历史快照、行业趋势、龙头股列表、龙头股详情和偏好配置覆盖了行业工作区的主要数据需求；聚类和轮动接口则提供补充观察视角。[1-2][6-7]",
        "异常返回需要尽量结构化。外部数据源失败时，后端不能只抛出原始异常，否则前端难以判断是完全不可用、部分字段缺失还是可以使用过期缓存。项目中保留降级说明和状态字段，就是为了让页面在异常场景下仍能为使用者提供可理解的反馈。":
            "异常返回需要保持结构化。外部数据源失败时，后端不能只抛出原始异常，否则前端无法判断是完全不可用、部分字段缺失，还是可以读取过期缓存。系统保留降级说明和状态字段，使页面在异常场景下仍能给出可理解反馈。",
        "详情弹窗承担结果追溯功能。行业详情给出趋势、成分股和覆盖信息，龙头股详情给出得分拆解、价格走势和财务字段。使用者从热力图进入详情后，可以沿着同一条数据链路继续检查，而不需要切换到另一套工具重新整理材料。":
            "详情弹窗用于追溯结果来源。行业详情展示趋势、成分股和覆盖信息，龙头股详情展示得分拆解、价格走势和财务字段。使用者从热力图进入详情后，可以沿同一条数据链路继续核对，而不必切换工具重新整理。",
        "测试过程中需要区分功能正确性和投资有效性。功能正确性关注接口能否返回、字段是否完整、页面能否交互、快照是否可回看；投资有效性则需要长期回测和交易成本假设。本文的测试重点是前者，后者作为未来研究方向保留。":
            "测试部分区分功能正确性和投资有效性。本文验证接口返回、字段完整性、页面交互和快照回看；长期收益表现、交易成本和换仓规则需要另行设计回测，作为后续研究保留。",
        "功能测试首先检查分析层。行业评分需要在字段完整时输出合理得分，在字段缺失时走回退路径，在波动率无法获得时使用代理值。龙头股评分则需要分别验证完整评分和快速评分，保证 core 与 hot 两类榜单语义不混乱。":
            "功能测试首先覆盖分析层。行业评分在字段完整时应输出有效分数，字段缺失时应走回退路径，波动率缺失时应启用代理值；龙头股评分分别验证完整链路和快速链路，确保 core 与 hot 两类语义保持区分。",
        "从论文材料角度看，持续跟踪功能还保证了样例可复核。第六章使用的热力图快照和龙头股样本均来自保存下来的固定文件；如果没有这些记录，表格数值会随着外部数据源更新而漂移，评阅时也难以回到同一时点检查。":
            "从论文材料看，持续跟踪功能保证样例可复核。第六章的热力图快照和龙头股样本均来自固定文件；没有这些记录，表格数值会随外部数据更新而漂移，评阅时也难以回到同一时点检查。",
        "测试环境选择本地运行，是因为毕业设计需要同时验证后端分析、前端展示和文档截图。后端负责接口和评分，前端负责交互与渲染，历史快照负责样例复核。三者必须放在同一环境里一起看，才能判断系统是否形成完整闭环。":
            "测试环境采用本地运行，便于同时观察后端分析、前端展示和文档截图。后端负责接口与评分，前端负责交互和渲染，历史快照负责样例复核；三者在同一环境中联动，才能判断系统是否形成闭环。",
        "为保证龙头股案例与热力图样例一样可复核，本文固定引用项目中保存的一条龙头股双榜单样本。结合 2026 年 4 月 20 日 12:20:54 保留的热力图样例可以看到，军工电子行业处于当期热点前列；根据 2026 年 4 月 21 日 12:55:01 记录的榜单结果，军工电子行业的 core 榜单前列主要为中直股份、中航西飞、中航机载，hot 榜单前列则出现中国卫星、威海广泰、中直股份；前者更偏向规模、估值和盈利质量的综合筛选，后者更强调短期涨幅与资金承接。该结果说明龙头股遴选并非对行业热度排序的简单复制，而是在已识别热门行业基础上进一步提供“核心资产”和“热点先锋”两类互补视角。":
            "龙头股案例同样采用固定样本。热力图样例保存于 2026 年 4 月 20 日 12:20:54，显示军工电子处于当期热点前列；2026 年 4 月 21 日 12:55:01 的榜单记录中，军工电子 core 前列为中直股份、中航西飞、中航机载，hot 前列为中国卫星、威海广泰、中直股份。两组名单不完全一致，说明龙头股遴选不是复述行业热度排序，而是在热点行业内部继续区分核心资产与短期活跃标的。",
        "因此，本文已把任务书中的理论梳理、模型设计、系统实现和结果验证落到论文与项目证据中，主线完整。":
            "由此可见，任务书要求并非只停留在文字说明中，而是分别落实到文献综述、评分模型、接口实现、页面截图和固定快照证据中。"
    }
    phrase_replacements = {
        ...
        "还有一个好处": "还有一项优势",
        "这样做的好处是": "这样做的优势在于",
        "行业和个股权重主要依据工程经验": "权重来自文献口径和本地样例调试",
        "影响排序稳健性": "影响不同窗口下的排序稳定性",
        "引入敏感性分析和滚动回测": "补充权重敏感性分析与滚动窗口检验",
        "部分行业成分股和财务字段可能降级": "部分成分股或财务字段会进入降级路径",
        "影响详情完整度": "影响详情页字段解释完整度",
        "补充数据源并记录缺失来源": "补充来源标签并记录缺失字段",
        "政策、新闻和公告暂未进入核心得分": "政策、新闻和公告仍作为后续扩展信息",
        "影响热点原因解释": "限制热点形成原因的解释深度",
        "加入文本情绪和事件标签": "加入政策事件、舆情标签和产业链线索",
        "固定快照不能替代长期收益检验": "固定快照只能说明样本时点表现",
        "限制策略有效性判断": "不能直接证明长期收益有效性",
        "构建换仓规则和样本外检验": "补充换仓规则、交易成本和样本外检验",
        "当前以本地研究原型为主": "当前主要面向本地论文复核和答辩展示",
        "限制多人协作和长期服务": "不适合多人并发和长期在线服务",
        "后续迁移数据库和任务调度": "后续再接入数据库、定时任务和权限控制",
        "明确本系统的数据来源、分析流程与关键技术": "对照 THS-first 数据源、评分流程和关键技术",
        "实现行业横截面评分、波动率代理与双路径个股评分": "对应 IndustryAnalyzer 与 LeaderStockScorer 的两条评分链路",
        "完成 THS-first、FastAPI、React、热力图快照与偏好持久化闭环": "对应 THS-first 适配、FastAPI 路由、React 页面、快照文件和 profile 偏好保存",
        "获取 THS 行业目录、行业摘要、成分股与财务等数据": "拉取 THS 行业树、摘要、成分股及财务字段",
        "THS-first 适配器主导，AKShare/Sina/腾讯补充": "优先 THS，异常时转 AKShare/Sina/腾讯",
        "计算热门行业得分并输出排序": "生成行业热度分和排序",
        "横截面评分、波动率代理、聚类与轮动分析": "动量/资金/活跃度/波动代理加权，聚类辅助",
        "在行业内部筛选代表性个股": "在行业成分股中筛选 core/hot 候选",
        "完整评分与快照快速评分": "完整六维评分和快照快速路径",
        "展示热力图、排行榜和详情弹窗": "展示热力图、榜单和详情弹窗",
        "React 仪表盘、热力图与详情联动": "React 页面联动热力图、榜单和弹窗",
        "保存观察行业、阈值和视图配置": "保存观察行业、阈值和视图状态",
        "按 profile 的 JSON 持久化与浏览器缓存": "profile JSON 与浏览器状态保存",
        "对照 THS-first 数据源、评分流程和关键技术": "核对 THS-first 来源、评分链路和技术栈",
        "对应 IndustryAnalyzer 与 LeaderStockScorer 的两条评分链路": "核对 IndustryAnalyzer 与 LeaderStockScorer",
        "对应 THS-first 适配、FastAPI 路由、React 页面、快照文件和 profile 偏好保存": "核对适配器、路由、页面、快照和 profile 配置",
        "快速评分链路可输出 20-95 展示得分，代理波动率与字段回退生效": "快速路径输出展示分，代理波动率和字段回退可用",
        "THS 主源可用，AKShare/Sina/腾讯补充链路与缓存兜底正常": "THS 主源、补充来源和缓存兜底均可用",
        "core/hot 榜单与详情接口均返回综合得分、维度拆解和原始字段": "core/hot 榜单和详情均返回得分、拆解及原始字段",
    }
