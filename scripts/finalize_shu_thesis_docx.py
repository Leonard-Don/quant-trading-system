...
DOC_PATH = Path(
    "/Users/leonardodon/PycharmProjects/PythonProject/output/doc/"
    "上海大学本科毕业论文_基于大数据的热门行业识别与龙头股遴选研究.docx"
)
...
OUTPUT_PDF_PATH = Path(
    "/Users/leonardodon/PycharmProjects/PythonProject/output/doc/"
    "上海大学本科毕业论文_基于大数据的热门行业识别与龙头股遴选研究.pdf"
)
LEGACY_DUPLICATE_PDF_PATH = Path(
    "/Users/leonardodon/PycharmProjects/PythonProject/output/doc/"
    "上海大学本科毕业论文_基于大数据的热门行业识别与龙头股遴选研究_送审版.pdf"
)
TMP_DOC_DIR = Path("/Users/leonardodon/PycharmProjects/PythonProject/tmp/docs")
...
FIGURE_IMAGES = {
    "图 5.1 行业热度总览界面": {
        "path": "/Users/leonardodon/PycharmProjects/PythonProject/docs/screenshots/industry-ranking-overview.png",
        "crop": (220, 250, 1400, 1110),
        "slug": "figure_5_1",
    },
    "图 5.2 行业热力图界面": {
        "path": "/Users/leonardodon/PycharmProjects/PythonProject/docs/screenshots/industry-heatmap-overview.png",
        "crop": (220, 150, 1400, 1110),
        "slug": "figure_5_2",
    },
    "图 5.3 龙头股详情界面": {
        "path": "/Users/leonardodon/PycharmProjects/PythonProject/docs/screenshots/leader-stock-detail.png",
        "crop": (300, 120, 1100, 950),
        "slug": "figure_5_3",
    },
}
TMP_ASSET_DIR = Path("/Users/leonardodon/PycharmProjects/PythonProject/tmp/docs/figure_assets")
CJK_FONT_SOURCES = (
...
def update_body_content(doc: Document) -> None:
    replacements = {
        ...
        "系统采用前后端分离架构。前端基于 React 与 Ant Design 构建行业热力图":
            "系统采用前后端分离架构。前端围绕行业热力图、行业排行榜、行业趋势与龙头股详情构建研究界面；后端基于 FastAPI 提供热门行业、行业成分股、热力图、行业趋势和龙头股等核心接口；分析层使用 Python 完成数据清洗、标准化和综合评分。数据侧采用 THS-first 适配策略，以同花顺行业目录和行业摘要作为主入口，并结合 AKShare、新浪财经和腾讯接口进行字段补齐与回退。",
        ...
        "多源数据采集是本系统实现的基础。结合项目代码，系统通过数据提供器工厂统一管理不同来源的数据接口。":
            "多源数据采集是本系统实现的基础。结合项目代码，行业子系统以 THS-first 适配策略组织不同来源的数据接口。对于 A 股行业分析任务，同花顺相关接口优先提供行业目录、行业摘要、资金流与领涨股等主数据，AKShare 负责补齐行业元数据、成分股、估值与财务信息，新浪财经和腾讯接口则承担部分回退与字段兜底职责。",
        ...
        "系统采用前后端分离架构，前端使用 React 与 Ant Design 构建行业热力图、行业排行榜、龙头股面板、行业趋势图和轮动对比图等交互页面，后端基于 FastAPI 提供数据接口与分析服务，核心分析逻辑由 Python 完成。数据层以 AKShare 获取 A 股行业分类、行业资金流、行业指数、个股估值和财务数据，同时引入新浪行业适配层作为回退机制，以提升系统运行中的稳定性和可用性。":
            "系统采用前后端分离架构，前端使用 React 与 Ant Design 构建行业热力图、行业排行榜、行业趋势图和龙头股详情等核心页面，后端基于 FastAPI 提供行业分析接口与偏好配置服务，核心分析逻辑由 Python 完成。行业数据层并非简单依赖 AKShare 单一数据源，而是由 THS-first 适配器主导：同花顺行业目录与行业摘要提供主要的行业热度底座，AKShare 负责行业元数据、成分股、财务与历史行情补齐，新浪财经和腾讯估值接口承担回退与字段兜底，从而提升行业模块的连通性与抗故障能力。",
        ...
        "系统数据流程可概括为：前端页面发起请求后，接口层根据请求类型调用行业分析器或龙头股评分器；分析器从主数据源获取行业与个股原始数据，必要时启用回退数据源；随后对数据进行清洗、字段统一和异常兜底；在标准化后计算行业或个股综合得分；最后将结果封装为统一响应结构返回前端，并在本地缓存和快照文件中记录必要信息。":
            "系统数据流程可概括为：前端页面通过 REST 请求调用行业接口；路由层优先检查端点缓存，随后根据请求类型调用 IndustryAnalyzer 或 LeaderStockScorer；分析层优先从 THS-first 适配器获取行业摘要、行业资金流和成分股快照，必要时再回退到 AKShare、新浪或腾讯接口，并允许使用过期缓存兜底；在字段统一、异常处理和标准化之后，系统计算行业或个股综合得分，将结果封装为统一响应结构返回前端，并把必要的历史快照与偏好配置持久化到本地文件或浏览器缓存。",
        ...
        "系统的数据层以数据提供器工厂为统一入口，对不同数据源进行注册、管理与切换。对于行业分析任务，AKShare 是主数据源，承担行业分类、行业资金流、行业指数、成分股、估值与财务信息的获取工作；新浪行业适配层则负责在部分接口失败或字段不完整时进行补充。":
            "系统的数据层以 THS-first 适配器作为行业分析主入口。该适配器优先利用同花顺行业目录与行业一览表获取行业名称、涨跌幅、资金流与领涨股等主数据，再由 AKShare 补齐行业元数据、成分股、财务与历史 K 线，新浪财经接口承担行业列表、成分股与行情兜底，腾讯财经接口则补单股估值核心字段。",
        "这种设计使系统形成了主数据源优先、备用数据源兜底的运行模式。对于面向真实市场数据的应用场景而言，这种容错机制比单纯依赖单一接口更具工程可用性。":
            "这种设计使系统形成了“同花顺主导、AKShare增强、新浪与腾讯兜底”的运行模式。对于面向真实市场数据的行业研究场景而言，多源适配比依赖单一接口更具工程可用性，也更能覆盖行业目录、资金流、成分股、估值和历史走势等不同类型的数据需求。",
        ...
        "为了提升接口稳定性，接口层额外设置了结果缓存，并在必要时使用过期缓存作为兜底返回。这一设计能够在外部数据源短暂波动时尽量保持页面可用。":
            "为了提升接口稳定性，路由层额外维护了 3 分钟端点缓存、30 分钟 parity 缓存以及完整版成分股异步构建机制，并在必要时使用过期缓存作为兜底返回。热力图历史数据还设置了最多 48 条记录和 2MB 文件大小上限，从而在外部数据源短暂波动时尽量保持页面可用。",
        ...
        "本文的系统测试基于项目当前本地环境展开。系统采用 Python 作为后端分析语言，FastAPI 作为接口框架，React 作为前端框架，数据源主要为 AKShare，并结合项目中保存的历史快照进行结果验证。由于本课题属于毕业设计性质，测试重点放在功能闭环、数据连通性、结果可解释性和页面展示效果，而非面向生产环境的极限压测。":
            "本文的系统测试基于项目当前本地环境展开。系统采用 Python 作为后端分析语言，FastAPI 作为接口框架，React 作为前端框架，行业主数据经由 THS-first 适配器提供，并在需要时调用 AKShare、新浪与腾讯接口补齐字段，同时结合项目中保存的历史快照进行结果验证。除样例快照分析外，项目还保留了行业评分快路径、适配器映射、偏好服务与前端行业页面的测试脚本，用于验证评分逻辑、回退逻辑和页面交互闭环。由于本课题属于毕业设计性质，测试重点放在功能闭环、数据连通性、结果可解释性和页面展示效果，而非面向生产环境的极限压测。",
        "为了增强论文结果分析的真实性，本文直接选取项目中已经保存的热力图历史快照作为样本。根据 2026 年 4 月 11 日 22:59:20 保存的五日窗口行业热力图数据，系统输出的前十个行业结果如表 6.2 所示。":
            "为了增强论文结果分析的真实性，本文直接选取项目中已经保存的热力图历史快照作为样本。根据 2026 年 4 月 11 日 22:59:20 保存的五日窗口行业热力图数据，表 6.2 展示了该样本时点按热力图默认顺序截取的前十个行业横截面字段，包括 total_score、5 日涨跌幅、资金流和换手率等信息。需要说明的是，这里给出的结果用于样例分析，并不构成完整回测结论。",
        ...
        "从工程角度看，本系统具有以下几个明显特征":
            "从工程角度看，本系统具有以下几个明显特征。第一，系统以 THS-first 适配器为主入口，并通过 AKShare、新浪和腾讯形成多源补齐链路，保证了行业数据的连通性。第二，行业评分与龙头股评分同时引入快路径与多级缓存机制，在保证响应速度的同时尽量维持评分口径一致。第三，系统输出不仅提供综合得分，还提供资金、波动率、市值来源和评分拆解等辅助信息，因此具有较好的结果解释性。",
        "在本次毕业设计与论文写作过程中":
    ...
    if test_heading is not None:
        ...
        intro_text = (
            ...
            "验证行业热度计算的快路径与慢路径回退、历史波动率优先级以及龙头股评分逻辑；接口与适配器测"
    ...
    sample_table = find_table_containing_text(doc, "行业名称")
    if sample_table is not None and len(sample_table.rows) >= 1:
...
def relayout_figures(doc: Document) -> None:
    ...
    body_width = int((body_section.page_width - body_section.left_margin - body_section.right_margin) * 0.92)
    ...
    for title, config in FIGURE_IMAGES.items():
        ...
        p_pr = image_para._element.get_or_add_pPr()
        page_break_before = OxmlElement("w:pageBreakBefore")
        p_pr.append(page_break_before)
        image_para.add_run().add_picture(str(output_path), width=Emu(body_width))
...
def normalize_reference_entries(doc: Document) -> None:
    entries = [
        ...
        "[7] Cavalcante R C, Brasileiro R C, Souza V L, et al. Computational Intelligence and Financial Markets: A Survey and Future Directions[J]. Expert Systems with Applications, 2016, 55: 194-211.",
        "[8] Fisher I, et al. Analytics for Financial Data: Methods and Applications[J]. Journal of Big Data Analytics in Finance, 2022, 3(1): 22-35.",
        "[9] McKinney W. Python for Data Analysis: Data Wrangling with Pandas, NumPy, and IPython[M]. 3rd ed. Sebastopol: O'Reilly Media, 2022.",
    ...
    for offset, entry in enumerate(entries):
        ...
        replace_paragraph_text(paragraph, entry)
...
def format_paragraphs(doc: Document) -> None:
    ...
    for idx, paragraph in enumerate(doc.paragraphs):
        ...
        if text == "摘  要":
            ...
            paragraph.paragraph_format.space_after = Pt(0)
            paragraph.paragraph_format.line_spacing = ABSTRACT_LINE_SPACING
            set_run_text_font(run, "黑体", Pt(18), bold=False)
            continue
        if text == "ABSTRACT":
            clear_paragraph(paragraph)
            ...
            paragraph.paragraph_format.space_after = Pt(0)
            paragraph.paragraph_format.line_spacing = ABSTRACT_LINE_SPACING
            set_run_text_font(run, "Times New Roman", Pt(18), bold=False)
            continue
        if text == "目  录":
